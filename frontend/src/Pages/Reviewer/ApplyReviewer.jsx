...
const ApplyReviewer = () => {
  // "How to Apply?" heading + content are managed from the Admin CMS.
  ...
  useEffect(() => {
    api
      .get("/api/site-content/reviewer-how-to-apply")
      .then((res) => {
        if (res.data) {
          setHowToApply({
            heading: res.data.heading || "How to Apply?",
            content:
              res.data.content || "Send your updated CV to support@ijaht.com",
          });
        }
      })
      .catch((err) => console.error("Reviewer content fetch error:", err));
  }, []);
